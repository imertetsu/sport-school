...
(vía `pago_cuota`) → inscripción → alumno → `alumno_tutor.responsable_pago=True` →
...
def _primera_cuota_de_pago(db: Session, pago_id: uuid.UUID) -> Cuota | None:
    """Una cuota cubierta por el pago (vía puente), para resolver alumno/inscripción."""
    return db.execute(
        select(Cuota)
...
def enviar_recibo_whatsapp(
    ...
    *,
    ...
    Flujo:
    1. Resuelve cuota → inscripción → alumno → tutor responsable de pago.
    ...
    nombre_alumno = _nombre_completo(alumno) if alumno is not None else "—"
    ...
    #   {{1}} nombre alumno, {{2}} monto, {{3}} escuela, {{4}} N° recibo, {{5}} enlace PDF.
