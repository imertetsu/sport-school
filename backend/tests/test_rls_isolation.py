...
def test_sin_contexto_no_devuelve_filas(app_engine: Engine, two_orgs: dict) -> None:
    ...
    with app_engine.connect() as conn:
        ...
        count = conn.execute(text("SELECT count(*) FROM alumno")).scalar_one()
    assert count == 0, "Sin contexto de tenant, alumno debe devolver 0 filas (fail-closed)"
...
def test_org_a_no_ve_filas_de_org_b(app_engine: Engine, two_orgs: dict) -> None:
    ...
    with app_engine.begin() as conn:
        ...
        rows = conn.execute(text("SELECT org_id FROM alumno")).scalars().all()

    org_ids = {str(r) for r in rows}
    assert org_a in org_ids, "Con org A fijada se deben ver sus alumnos"
    assert org_b not in org_ids, "Con org A fijada NO se deben ver alumnos de org B"
